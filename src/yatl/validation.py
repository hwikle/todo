...
from yatl.model import Category, CategoryMembership, Task, TodoList
                ...
                if task["completed"] and not dependency["completed"]:
                    issues.append(
                        Issue(
                            "error",
                            location,
                            f"dependency {dependency['name']!r} is incomplete",
                        )
                    )
                    ...
                    if task_rank < len(self.priority_policy.order) - 1:
                        issues.append(
                            Issue(
                                ...
                                f"than task priority {task['priority']!r}",
                            )
                        )
                    ...
                    if dependency_rank > task_rank:
                        issues.append(
                            Issue(
                                "error",
                                location,
                                f"dependency {dependency['name']!r} priority "
                                f"{dependency['priority']!r} is less urgent than "
